...
__global__ void my_add(int32_t size, const float* __restrict__ in1, 
  const float* __restrict__ in2, float* __restrict__ out){
  int32_t tid = threadIdx.x + blockDim.x * blockIdx.x;
  if(tid>size){
    return;
  }
  float in_val1=in1[tid], in_val2=in2[tid];
  out[tid] = in_val1 + in_val2;
}
...
void add_kernel_cu(const tensor::Tensor& input1, const tensor::Tensor& input2,
  ...
  if (stream) {
    ...
    my_add<<<block_num, thread_num, 0, stream_>>>(
        size, input1.ptr<float>(), input2.ptr<float>(), const_cast<float*>(output.ptr<float>()));
  } else {
    my_add<<<block_num, thread_num>>>(size, input1.ptr<float>(), input2.ptr<float>(),
